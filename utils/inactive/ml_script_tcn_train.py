...
def add_weighted_error_feature(df, nominal_state_lookup, mu):
    # --- Tunable Weights ---
    alpha = 1.0  # Prioritizes position error
    beta = 0.1   # Prioritizes velocity error
...
def train_model_for_segment(segment_name, X_train_data, y_train_data, scaler_X, scaler_y):
    ...
    X_train_scaled = scaler_X.transform(X_train_data[:, :-2])
    ...
    model = TCN_MANN(input_size=X_train_scaled.shape[1], hidden_size=hidden_size, output_size=y_train_scaled.shape[1], num_layers=num_layers).to(device)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f"trained_model_tcn_{segment_name}.pt")
        else:
            epochs_no_improve += 1
...
if __name__ == "__main__":
    ...
    X_train_max, y_train_max, X_train_min, y_train_min = [], [], [], []
    ...
    base_feature_cols = ['t', 'p', 'f', 'g', 'h', 'k', 'L', 'mass', 'c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7']
    bookkeeping_cols = ['bundle_idx', 'sigma_idx']
    
    df_X_max = pd.DataFrame(X_full_max, columns=base_feature_cols + bookkeeping_cols)
    ...
    df_X_min = pd.DataFrame(X_full_min, columns=base_feature_cols + bookkeeping_cols)
    ...
    df_X_max_featured = add_weighted_error_feature(df_dedup_max, nominal_state_max, mu)
    ...
    df_X_min_featured = add_weighted_error_feature(df_dedup_min, nominal_state_min, mu)
    
    final_feature_cols = base_feature_cols + ['weighted_error']
    ...
    # --- Step 4: Removal of Appended sigma0 Rows ---
    is_sigma0_max = X_max_with_features[:, -2] == 0
    is_zero_cov_max = np.all(np.isclose(X_max_with_features[:, 8:15], 0.0, atol=1e-12), axis=1)
    X_max_final = X_max_with_features[~(is_sigma0_max & is_zero_cov_max)]
    y_max_final = y_max_dedup[~(is_sigma0_max & is_zero_cov_max)]
    
    is_sigma0_min = X_min_with_features[:, -2] == 0
    is_zero_cov_min = np.all(np.isclose(X_min_with_features[:, 8:15], 0.0, atol=1e-12), axis=1)
    X_min_final = X_min_with_features[~(is_sigma0_min & is_zero_cov_min)]
    y_min_final = y_min_dedup[~(is_sigma0_min & is_zero_cov_min)]
    ...
    scaler_X = StandardScaler().fit(combined_X_for_scaling[:, :-2])
