...
def main():
    ...
    # Pick bundle for MC + Mahalanobis diagnostics
    ...
    for stride in time_strides:
        ...
        for config in uncertainty_configs:
            ...
            # === Mahalanobis Diagnostic only for selected bundle and max-width interval ===
            if stride == 1 and name == "baseline":
                ...
                if sigma0_row is not None:
                    ...
                    time_idx = np.where(np.isclose(np.array(backTspan)[::-1], t_k))[0][0]
                    P_final = P_hist[bundle_idx_for_diag][time_idx][:3, :3]
                    inv_cov = inv(P_final)
                ...
                # After computing P_final for sigma trajectories
                print(f"\nSigma Point Covariance Matrix at t = {t_k:.6f} (position only):")
                print(P_final)

                # After MC trajectory is generated
                mc_P_final = mc_P_hist[0][1][:3, :3]  # Time index 1 = t_{k+1}
