...
def compute_nominal_trajectory_params():
    ...
    equations, and it returns the solution and the calculated final time of flight.
    ...
    while normfval >= 1e-4:
        ...
        p_sol = scipy.optimize.fsolve(calc_residuals_nominal, y, args=data, xtol=1e-12, maxfev=10000)
        
        # Compute the residuals for the current solution
        fval = calc_residuals_nominal(p_sol, s0, s_t, mu, F, c, m0, g0)
    ...
    tfound = 0.5 * ((tf_g_UB + tf_g_LB) + (tf_g_UB - tf_g_LB) * np.sin(wrapTo2Pi(p_sol[-1])))

    # Return the optimized solution (p_sol) and final time of flight (tfound)
    return p_sol, tfound
